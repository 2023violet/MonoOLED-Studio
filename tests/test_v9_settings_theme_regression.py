...
def test_theme_refresh_signature_tracks_theme_and_metrics() -> None:
    ...
    assert 'stylesheet = build_adaptive_stylesheet(density, ui_scale=ui_scale)' in body
    assert 'if app.styleSheet() != stylesheet:' in body
    assert body.count('app.setStyleSheet(stylesheet)') == 1
