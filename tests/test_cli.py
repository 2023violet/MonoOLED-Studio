...
def test_simulate_command_runs_generic_scene_and_writes_realtime_jsonl(tmp_path):
    log = tmp_path / 'session.jsonl'
    result = run_cli('simulate', '--steps', '6', '--interval', '0', '--log', str(log))
    assert result.returncode == 0, result.stdout + result.stderr
    assert 't=0005' in result.stdout
    assert 'phase=running' not in result.stdout  # V12 generic default has no implicit runtime state
    assert 't=0006' in result.stdout
    text = log.read_text(encoding='utf-8')
    assert '"event": "FRAME"' in text
    assert '"event": "RESET"' in text
    assert '"event": "STATE"' not in text  # no state transition exists in the V12 neutral default scene
