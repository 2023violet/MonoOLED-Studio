...
def test_v1242_release_identity_and_current_documentation_contract():
    ...
    assert manifest['release_name'] == f'V{version} Pixel Studio Experience Release'
...
def test_current_windows_and_user_docs_reference_v100_public_asset():
    ...
    assert f'# V{VERSION} Pixel Studio Experience Release Delivery' in (ROOT / 'DELIVERY_README.md').read_text(encoding='utf-8')

def test_current_root_readme_and_about_surface_use_v100_identity():
    readme = (ROOT / 'README.md').read_text(encoding='utf-8')
    assert readme.startswith(f'# MonoOLED Studio V{VERSION} — Pixel Studio Experience Release')
