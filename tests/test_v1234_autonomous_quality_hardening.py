...
def _function_body(source: str, name: str) -> str:
    marker = f'        def {name}' if f'        def {name}' in source else f'    def {name}'
    start = source.index(marker)
    rest = source[start + len(marker):]
    indent = marker.split('def', 1)[0]
    next_pos = rest.find(f'\n{indent}def ')
...
def test_screen_change_preloads_scene_before_mutating_project_active_screen():
    body = _function_body(_text('gui_project_mixin.py'), '_project_screen_changed')
...
def test_pixel_identity_change_is_rekeyed_without_fake_save_side_effects():
    ...
    assert 'self._editor_open_pixel_studio()' in open_body
...
def test_open_scene_validates_target_before_unsaved_prompt():
    body=_function_body(_text('gui_project_mixin.py'),'_project_open_scene_dialog')
...
def test_open_project_validates_target_before_unsaved_prompt():
    body=_function_body(_text('gui_project_mixin.py'),'_project_open_project_dialog')
    ...
    loader=_function_body(_text('gui_project_mixin.py'),'_project_load_project_candidate')
...
def test_delete_screen_preloads_fallback_before_removing_current_screen():
    body=_function_body(_text('gui_project_mixin.py'),'_project_delete_screen')
...
def test_last_project_preference_persistence_is_nonfatal_and_shared():
    source=_text('gui_project_mixin.py')
    helper=_function_body(source,'_project_remember_last_project')
    assert "save=False" in helper
    assert 'except OSError as exc' in helper
    assert "PREFERENCES_SAVE_FAIL" in helper
    commit=_function_body(source,'_project_commit_project_candidate')
    assert 'self._remember_last_project(str(project.path))' in commit
    scene=_function_body(source,'_project_open_scene_dialog')
...
def test_new_font_pack_refuses_to_overwrite_existing_manifest_and_handles_io_errors():
    body=_function_body(_text('gui_resource_mixin.py'),'_resource_new_font_pack')
    assert "fontpack.json" in body
    assert "root / 'fontpack.json'" in body
    assert 'except Exception as exc: self._show_error(str(exc)); return' in body
...
def test_open_font_lab_requires_existing_manifest_and_catches_constructor_errors():
    ...
    assert 'self._editor_open_font_lab(root)' in body
...
def test_main_open_pixel_studio_catches_corrupt_image_constructor_error():
    ...
    assert 'self._editor_open_pixel_studio()' in body


def test_bitmap_text_insert_surfaces_element_validation_errors_instead_of_leaking():
    body=_function_body(_text('gui_resource_mixin.py'),'_resource_insert_bitmap_text')
