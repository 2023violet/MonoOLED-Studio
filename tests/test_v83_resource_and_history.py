...
def test_editor_session_reuses_resources_across_renders():
    ...
    # Resource-cache reuse must be exercised against a scene that actually
    # contains bitmap/font-backed elements. The user's active project screen
    # is mutable workspace state and is not a valid prerequisite for this test.
    scene=load_scene(SIM/'scenes'/'main_scene.json', project_root=SIM.parent)
...
def test_windows_launcher_source_has_gui_startup_validation_and_runtime_env_priority():
    # V12 source packages deliberately do not ship the obsolete runtime-locator launcher.
    root = SIM.parent
    ...
    assert '--startup-smoke' in builder
