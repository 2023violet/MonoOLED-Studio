...
def test_preference_delta_is_effect_scoped_and_does_not_mark_renderer_for_ui_only_changes():
    ...
    # Theme Closure V10.1 keeps color_theme only for file/API compatibility.
    assert not d2.theme_changed and d2.ui_metrics_changed
    assert d2.effects == frozenset({'metrics'})
    assert not d2.requires_product_render

    after3=_runtime(**{'appearance.theme_mode':'dark'})
    d3=PreferenceDelta.between(before,after3)
    assert d3.theme_changed
    assert d3.effects == frozenset({'theme'})
    assert not d3.requires_product_render
...
def test_ui_metrics_scale_typography_icons_and_navigation_not_only_control_height():
    ...
    for key in ('control','row','font_display','font_body','font_metadata','icon','gap','panel_margin','nav_min','inspector_min'):
