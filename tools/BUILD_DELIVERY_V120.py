...
TRANSIENT = {
    '.git', '.pytest_cache', '.venv', '.venv-build', '.artifacts',
...
def managed_files() -> list[Path]:
    return sorted(
        (p for p in ROOT.rglob('*') if include(p) and p.name != 'SHA256SUMS.txt'),
...
def write_sums() -> int:
    ...
    for path in files:
        ...
        if not rel.isascii():
            raise RuntimeError(f'non-ASCII V12 release path: {rel}')
...
def build_zip(target: Path) -> tuple[int, str]:
    write_sums()
    files = sorted(
        (p for p in ROOT.rglob('*') if include(p)),
        key=lambda p: p.relative_to(ROOT).as_posix(),
    )
    target = Path(target).resolve()
    ...
    with zipfile.ZipFile(target, 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path in files:
            rel = path.relative_to(ROOT).as_posix()
            if not rel.isascii():
                raise RuntimeError(f'non-ASCII V12 ZIP path: {rel}')
    ...
    with zipfile.ZipFile(target, 'r') as archive:
        ...
        for item in infos:
            ...
            if not item.filename.isascii():
                raise RuntimeError(f'non-ASCII V12 ZIP path: {item.filename}')
...
def main() -> int:
    ...
    print(f'PASS: V12 source ZIP entries={count}, non-ASCII paths=0')
