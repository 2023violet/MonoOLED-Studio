...
SIM=ROOT/'src'
sys.path.insert(0,str(SIM)); sys.path.insert(0,str(ROOT/'tools'))
...
def interactive_hot_path():
    _project,scene=_load_source(str(ROOT/'test_assets/projects/curing_lite/project.oled.json'))
...
def launcher_contract():
    ...
    assert (ROOT/'tools/CREATE_RUNTIME_ENV.bat').exists()
