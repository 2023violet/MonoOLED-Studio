...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument('output',nargs='?',default=str(ROOT.parent/'MonoOLED_Studio_v8.0_Unified_OLED_Workspace_Complete_Delivery_2026-08-23.zip')); args=p.parse_args()
    target=Path(args.output).resolve(); count,digest,nonascii=build_zip(target)
    print(f'PASS: managed files={count-1}, ZIP entries={count}, non-ASCII UTF-8 entries={nonascii}')
