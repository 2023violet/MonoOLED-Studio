...
class QtAutomationBridge(QObject):
    ...
    def stop(self):
        ...
        while True:
            try:
                request, done, box = self._requests.get_nowait()
            except queue.Empty:
                break
            box['response'] = {
    ...
    def _dispatch_from_thread(self, request):
        ...
        self._requests.put((request, done, box))
        if not done.wait(10):
            return {
                'jsonrpc': '2.0', 'id': request.get('id'),
    ...
    def _drain(self):
        for _ in range(32):
            try:
                req, done, box = self._requests.get_nowait()
            except queue.Empty:
                return
            response = dispatch_json_rpc(self.service, req)
