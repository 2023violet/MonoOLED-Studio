...
def test_pytest_selected_automation_module_collects_from_repo_root_without_pythonpath():
    ...
    result = subprocess.run(
        [sys.executable, '-m', 'pytest', 'OLED模拟器/tests/test_automation_reliability_v842.py', '--collect-only', '-q'],
        cwd=ROOT,
        env=env,
        text=True,
        capture_output=True,
...
spec=importlib.util.spec_from_file_location('runner', root/'Developer_Tools'/'RUN_WINDOWS_TEST_GROUPS.py')
