...
def test_runtime_paths_in_source_tree_are_platform_native(tmp_path):
    module_file = tmp_path / 'project' / 'OLED模拟器' / 'scene.py'
    ...
    assert scenes == tmp_path / 'project' / 'OLED模拟器' / 'scenes'
    assert logs == tmp_path / 'project' / 'OLED模拟器' / 'logs'
...
def test_runtime_paths_in_pyinstaller_onedir_bundle_are_platform_native(tmp_path):
    ...
    assert root == bundle
    assert scenes == bundle / 'OLED模拟器' / 'scenes'
