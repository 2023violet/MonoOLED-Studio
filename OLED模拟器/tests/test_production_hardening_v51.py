...
def test_release_dev_requirements_include_real_qt_gui_test_stack():
    dev = (SIM / 'requirements-dev.txt').read_text(encoding='utf-8')
    assert 'pytest-qt==4.5.0' in dev


def test_windows_workflow_has_dpi_and_real_gui_test_matrix():
    workflow = (SIM.parent / '.github' / 'workflows' / 'build-windows-exe.yml').read_text(encoding='utf-8')
...
def test_windows_builder_has_soak_gate():
    script = (SIM.parent / 'Developer_Tools' / 'BUILD_WINDOWS_EXE.bat').read_text(encoding='utf-8')
