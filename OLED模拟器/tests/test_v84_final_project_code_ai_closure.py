...
def test_v84_release_identity_and_final_documents():
    assert (SIM / 'VERSION').read_text(encoding='utf-8').strip().startswith('8.4.')
    gui = (SIM / 'gui.py').read_text(encoding='utf-8')
    assert "APP_VERSION = '8.4." in gui
    manifest = json.loads((ROOT / 'DELIVERY_MANIFEST.json').read_text(encoding='utf-8'))
    assert str(manifest['version']).startswith('8.4.')
    assert (SIM / 'FINAL_PROJECT_CODE_AI_CLOSURE_V84.md').is_file()
    for name in (
