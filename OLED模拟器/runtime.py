...
class SceneRuntime:
    ...
    def step(self, seconds: int = 1) -> dict:
        if not isinstance(seconds, int) or isinstance(seconds, bool) or seconds < 0:
            raise ValueError('step seconds must be a non-negative integer')
        for _ in range(seconds):
            now = self.elapsed + 1

            # Recurring actions observe the state that existed during the
            # preceding second. This keeps a newly-entered running state at
            # its initial countdown value for one full second and makes the
            # 5..305 example produce exactly 300 decrements.
            for action in self.scene.get('timeline', []):
