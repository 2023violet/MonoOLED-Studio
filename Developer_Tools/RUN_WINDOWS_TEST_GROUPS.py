...
def _pytest_group(python: str, files: list[Path], *, report_dir: Path, tag: str, timeout: int, env: dict[str,str], no_skips: bool) -> int:
    ...
    if no_skips and counts['skipped']:
        ...
        return 3
    if no_skips:
        verify = ROOT / 'Developer_Tools' / 'VERIFY_JUNIT_NO_SKIPS.py'
...
def run_qt(args, env: dict[str,str], report_dir: Path) -> int:
    ...
    for scale in scales:
        ...
        for path in qt:
            ...
            rc = _pytest_group(args.python, [path], report_dir=report_dir, tag=tag, timeout=args.qt_timeout, env=scale_env, no_skips=True)
            if rc:
                return rc
        if args.qt_smokes:
            for smoke_name, smoke_args in (
                ...
                rc=_run_process([args.python, str(SIM/'gui.py'), *smoke_args], env=scale_env, timeout=args.qt_timeout, log=report_dir/f'{tag}.log')
                if rc:
                    return rc
    return 0
