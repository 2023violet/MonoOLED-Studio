...
def include(path: Path) -> bool:
    ...
    if len(rel.parts)>=3 and rel.parts[0]=='OLED模拟器' and rel.parts[1]=='reports':
...
def build_zip(target: Path) -> tuple[int,str,int]:
    ...
    digest=sha(target);target.with_suffix(target.suffix+'.sha256').write_text(f'{digest}  {target.name}\n',encoding='ascii')
    return len(files),digest,nonascii


def main()->int:
    parser=argparse.ArgumentParser();parser.add_argument('output',nargs='?',default=str(ROOT.parent/'MonoOLED_Studio_v8.4.3_Windows_Release_Real_Qt_GA_Closure_Complete_Delivery_2026-08-24.zip'));args=parser.parse_args()
