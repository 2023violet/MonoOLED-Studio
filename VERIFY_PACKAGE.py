...
def main()->int:
    ...
    if manifest.get('release_name')!=RELEASE: return fail('unexpected V8.4 release name')
    ...
    modules=(
        ...
        'resource_cache.py','atomic_io.py','diagnostics.py')
    for rel in modules:
        if not (SIM/rel).is_file(): return fail(f'V8.4 module missing: {rel}')

    documents=('FINAL_VERIFICATION_REPORT.md','TEST_MATRIX_V84.md','FINAL_PROJECT_CODE_AI_CLOSURE_V84.md','CODE_AI_AUTOMATION_API_V1.md','AUTOMATION_API_V1.json','TEST_MATRIX_V83.md','RELIABILITY_PERFORMANCE_CLOSURE_V83.md','USER_GUIDE_CN.md')
    ...
    for marker in ("APP_VERSION = '8.4.0'",'def run_startup_smoke(','--startup-smoke','CORE CHECK PASS','PreferenceDelta','editor_registry.apply_runtime_delta'):
    ...
    for marker in ('VERIFY_JUNIT_NO_SKIPS.py','--startup-smoke','VERIFY_V82_STRESS.py','VERIFY_V83_STRESS.py','VERIFY_V84_FINAL.py'):
        if marker not in builder: return fail(f'Windows V8.4 zero-skip/build marker missing: {marker}')
    for rel in ('Developer_Tools/VERIFY_V83_STRESS.py','Developer_Tools/VERIFY_V84_FINAL.py','Developer_Tools/BUILD_DELIVERY_V84.py','Developer_Tools/EXPORT_AUTOMATION_API_V1.py','Developer_Tools/VERIFY_JUNIT_NO_SKIPS.py','Developer_Tools/CREATE_RUNTIME_ENV.bat','Developer_Tools/RUN_MONOOLED_DIAGNOSTIC.bat'):
    ...
    if automation_contract.get('api_version')!='1.0.0': return fail('Automation API contract version must be 1.0.0')
    automation_source=(SIM/'automation_service.py').read_text(encoding='utf-8')
    for marker in ('AUTOMATION_API_VERSION = \'1.0.0\'','project.open_screen','render.all_states','validate.all_states','pixel.create','export.code_ai_handoff'):
        if marker not in automation_source: return fail(f'Automation API 1.0 marker missing: {marker}')
    builder=(ROOT/'Developer_Tools/BUILD_WINDOWS_EXE.bat').read_text(encoding='utf-8')
    if 'test_qt_v84_project_automation.py' not in builder or 'VERIFY_V84_FINAL.py' not in builder:
        return fail('Windows V8.4 Code-AI/Real-Qt gate markers missing')
    ...
    print('PASS: version=8.4.0, frozen product assets=464/464, Golden=14/14 x 512B')
    print('PASS: V8.3 reliability/performance + Automation API 1.0 Project/Code-AI closure + zero-skip Windows Real-Qt GA gates present')
    return 0
