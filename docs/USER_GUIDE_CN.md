...
普通用户下载 `MonoOLEDStudio_v1.2.3_Windows_x64.zip`，解压后运行 `MonoOLEDStudio\MonoOLEDStudio.exe`，不需要 Python。开发者可使用 `tools\BUILD_WINDOWS_QUICK.bat`；正式发布认证使用 `tools\BUILD_WINDOWS_GA.bat` 和 Real-Qt 分组测试。
