...
class AutoSaveManager:
    ...
    def _prune_retention(self) -> None:
        ...
        for old in snaps[:-self.keep]:
            try:
                ...
                # Retention cleanup is best-effort. A busy old snapshot must not
                # turn a successful fresh autosave into a false failure.
                continue
    ...
    def recovery_candidate(self) -> Path | None:
        ...
        try:
            ...
            # Recovery prompting is safer than crashing startup when metadata is
            # temporarily unavailable (sharing violation, removable media, race).
            return latest
