...
def _validate_frame_name(name: str) -> str:
    ...
    if not value or value in {'.','..'} or value.rstrip(' .') != value:
        raise ValueError(f'unsafe frame name: {name!r}')
...
def _prune_stale_frames(directory: Path, suffix: str, keep: set[str]) -> None:
...
def _validate_frame_names(states: Mapping[str, dict]) -> dict[str,str]:
    out: dict[str,str]={}
    folded: dict[str,str]={}
    for raw_name in states:
        if not isinstance(raw_name,str):
            raise ValueError(f'frame name must be a string: {raw_name!r}')
        safe=_validate_frame_name(raw_name)
        key=safe.casefold()
        previous=folded.get(key)
        if previous is not None and previous != safe:
            raise ValueError(f'frame name collision on Windows: {previous!r} vs {safe!r}')
        folded[key]=safe; out[raw_name]=safe
    return out
...
def export_scene(scene: dict, output_dir: str | Path, states: Mapping[str, dict], *, progress=None, cancel=None) -> ExportSummary:
    frame_names=_validate_frame_names(states)
    ...
    if any(has_blockers(list(fs)) for fs in all_findings.values()):
        ...
        atomic_write_text(output_dir / "validation_report.md", report)
    ...
    for render_index, name in enumerate(sorted(states)):
        ...
        bin_path = golden_dir / f"{safe_name}.bin"
        png_path = reference_dir / f"{safe_name}.png"
        atomic_write_bytes(bin_path, raw)
        ...
        frame_contracts[name] = {
            ...
            "golden": f"golden/{safe_name}.bin",
            "reference": f"reference/{safe_name}.png",
    ...
    atomic_write_text(output_dir / "validation_report.md", _validation_markdown(all_findings))

    _prune_stale_frames(golden_dir, '.bin', {f'{name}.bin' for name in frame_names.values()})
    _prune_stale_frames(reference_dir, '.png', {f'{name}.png' for name in frame_names.values()})
