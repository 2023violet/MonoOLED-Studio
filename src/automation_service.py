    ...
    def call(self, method, params=None, *, expected_revision=None, transaction=None):
        with self._lock:
            ...
            if method == 'pixel.create':
                ...
                self._register_pixel_doc(did, doc, path)
            ...
            if method == 'pixel.close':
                did = str(params['document_id']); doc = self._pixel_doc(did)
                if doc.dirty and not bool(params.get('discard', False)):
                    raise UnsavedChangesError(f'unsaved pixel document: {did}; save first or use discard=true')
                self.pixel_documents.pop(did, None)
                self.pixel_paths.pop(did, None)
                return self._result(document_id=did, closed=True)
            ...
            if method == 'font.set_metrics':
                root = self._font_root(params['font_id']); pack = FontPack.load(root); pack.set_metrics(baseline=int(params.get('baseline', pack.baseline)), advance=int(params.get('advance', pack.advance))); pack.save(); self._changed(method, transaction); return self._result(font_id=root.relative_to(self.project_root).as_posix(), baseline=pack.baseline, advance=pack.advance)
            ...
            if method == 'job.release':
                return self._result(**self._jobs.release(str(params['job_id'])))

            if method == 'session.events':
                since = max(0, int(params.get('since', 0)))
                retained_from = self._event_base
                dropped_before = max(0, retained_from - since)
                start = max(0, since - retained_from)
                next_cursor = retained_from + len(self.events)
                return self._result(
                    events=deepcopy(self.events[start:]),
