...
class PixelDocument:
    ...
    def from_image(cls, path: str | Path, *, threshold: int = 128, invert: bool = False, max_pixels: int = 4_194_304, max_dimension: int = 4096) -> 'PixelDocument':
        ...
        try:
            ...
            raise ValueError(f'unsupported or corrupt image: {target}') from exc
        rows=[]
        for y in range(h):
            row=[]
            for x in range(w):
    ...
    def save_c_header(self, path: str | Path, symbol: str = 'oled_bitmap') -> Path:
        target=Path(path); return atomic_write_text(target,self.to_c_header(symbol),encoding='utf-8')
    ...
    def save_png(self, path: str | Path) -> Path:
        ...
        buf=BytesIO(); img.save(buf,format='PNG',optimize=False); atomic_write_bytes(target,buf.getvalue()); self.dirty=False; return target

    def save_bin(self, path: str | Path) -> Path:
        target=Path(path); return atomic_write_bytes(target,self.to_vlsb())
