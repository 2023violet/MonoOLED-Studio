...
from PySide6.QtGui import QColor, QIcon, QImage, QPainter, QPainterPath, QPen, QPixmap, QKeySequence, QShortcut, QFont
from PySide6.QtWidgets import (
    QCheckBox, QDialog, QFileDialog, QFormLayout, QGridLayout, QHBoxLayout, QInputDialog,
...
def _tool_icon(name: str, color: str, size: int = 16) -> QIcon:
    ...
    return studio_icon(name, color, max(12,int(size)))
...
class ImageImportDialog(QDialog):
    def __init__(self, path: str | Path, tr: Translator, parent=None):
        ...
        form.addRow(tr('pixel.import.threshold_mode'),self.threshold_mode);form.addRow(tr('pixel.import.threshold'), self.threshold);form.addRow('R',self.red);form.addRow('G',self.green);form.addRow('B',self.blue); form.addRow('', self.invert); layout.addLayout(form)
...
class PixelRulerStrip(QWidget):
    """Viewport-edge ruler showing canvas-pixel indices for the pixel canvas.

    Display-only by product decision: it mirrors zoom and scroll state that the
    window pushes in via ``sync()`` and never interacts with the canvas hot
    paths.  ``orientation`` is 'horizontal', 'vertical', or 'corner' (blank
    fill matching the strips).
    """
    ...
    def __init__(self, orientation: str, parent=None):
        ...
        self.theme_name = 'monooled-light'
        if orientation == 'horizontal':
            self.setFixedHeight(RULER_STRIP_PX)
        elif orientation == 'vertical':
            self.setFixedWidth(RULER_STRIP_PX)
        else:
            self.setFixedSize(RULER_STRIP_PX, RULER_STRIP_PX)

    def sync(self, document_size: int, zoom: int, scroll_offset: int, viewport_size: int, theme_name: str):
        state = (max(0, int(document_size)), max(1, int(zoom)), max(0, int(scroll_offset)),
                 max(0, int(viewport_size)), theme_name)
        if state == (self.document_size, self.zoom, self.scroll_offset, self.viewport_size, self.theme_name):
            return
        self.document_size, self.zoom, self.scroll_offset, self.viewport_size, self.theme_name = state
        self.update()

    def _tick_step(self) -> int:
        step = 8
        while step * self.zoom < 40:
            step *= 2
        return step

    def _first_tick(self) -> int:
        return (self.scroll_offset // self.zoom // self._tick_step()) * self._tick_step()

    def paintEvent(self, event):
        theme = get_theme(self.theme_name)
        painter = QPainter(self)
        painter.fillRect(self.rect(), QColor(theme['surface.toolbar']))
        painter.setPen(QPen(QColor(theme['text.muted']), 1))
        painter.setFont(QFont('Segoe UI', 7))
        step = self._tick_step()
        first = self._first_tick()
        if self.orientation == 'horizontal':
            last = min(self.document_size, first + self.viewport_size // self.zoom + step)
            for value in range(first, last + 1, step):
                x = value * self.zoom - self.scroll_offset
                painter.drawLine(x, self.height() - 5, x, self.height() - 1)
                painter.drawText(x - 16, 0, 32, self.height() - 5, Qt.AlignHCenter | Qt.AlignBottom, str(value))
        elif self.orientation == 'vertical':
            last = min(self.document_size, first + self.viewport_size // self.zoom + step)
            for value in range(first, last + 1, step):
                y = value * self.zoom - self.scroll_offset
                painter.drawLine(self.width() - 5, y, self.width() - 1, y)
                painter.drawText(0, y - 6, self.width() - 6, 12, Qt.AlignRight | Qt.AlignVCenter, str(value))
        painter.end()
...
class PixelCanvas(QWidget):
    ...
    def _scaled_fill_pixmap(self):
        """Document rendered 1:1 through PIL, then nearest-neighbour scaled.

        One C-level pass over the pixels replaces the per-pixel drawRect loop
        that made every zoom notch a multi-tens-of-milliseconds rebuild on
        large canvases.
        """
        w, h = self.document.width, self.document.height
        z = self.zoom
        # Expand each pixel to a z x z block with two binary bytes.replace passes
        # and row repetition: everything stays in C-level bytes operations, so no
        # intermediate 1:1 image and no QPixmap.scaled() copy are needed.  The
        # 0/1 bytes are wrapped as an indexed QImage whose color table maps them
        # to the configured background/fill colors.
        zero_run = b'\x00' * z; one_run = b'\x01' * z
        raw = b''.join(
            (bytes(row).replace(b'\x00', zero_run).replace(b'\x01', one_run)) * z
            for row in self.document.pixels
        )
        bg = QColor(self.background_color); fg = QColor(self.fill_color)
        qimage = QImage(raw, w * z, h * z, w * z, QImage.Format_Indexed8)
        qimage.setColorTable([0xFF000000 | bg.rgb(), 0xFF000000 | fg.rgb()])
    ...
    def _base_pixmap(self):
        ...
        painter.drawPixmap(0, 0, self._scaled_fill_pixmap())
        if self.show_grid:
    ...
    def _patch_base_cache(self, bounds):
        ...
        if self._base_cache is not None:
            painter=QPainter(self._base_cache); painter.fillRect(damage,QColor(self.background_color)); painter.setPen(Qt.NoPen); painter.setBrush(QColor(self.fill_color))
    ...
    def paintEvent(self,event):
        ...
        if self.pixel_border_color:
            rect=event.rect()
            x0=max(0,rect.x()//z); y0=max(0,rect.y()//z)
            x1=min(self.document.width-1,rect.right()//z); y1=min(self.document.height-1,rect.bottom()//z)
            if x1>=x0 and y1>=y0:
                path=QPainterPath()
                for y in range(y0,y1+1):
                    row=self.document.pixels[y]
                    for x in range(x0,x1+1):
                        if row[x]: path.addRect(x*z,y*z,z,z)
                painter.setPen(QPen(QColor(self.pixel_border_color),1)); painter.setBrush(Qt.NoBrush); painter.drawPath(path)
        ...
        if self._key_cursor is not None:
            kx,ky=self._key_cursor; color=QColor(t['canvas.selection'])
            painter.setPen(QPen(color,2,Qt.SolidLine)); painter.setBrush(Qt.NoBrush)
            painter.drawRect(kx*z+1,ky*z+1,max(1,z-2),max(1,z-2))
        painter.end()
    ...
    def keyPressEvent(self,event):
        key=event.key()
        if key==Qt.Key_Space and not event.isAutoRepeat():self._space_down=bool(self.space_pan_enabled); event.accept(); return
        moves={Qt.Key_Left:(-1,0),Qt.Key_Right:(1,0),Qt.Key_Up:(0,-1),Qt.Key_Down:(0,1)}
        if key in moves and not event.isAutoRepeat():
            event.accept()
            if self._key_cursor is None:
                self._set_key_cursor((0,0)); return
            dx,dy=moves[key]; x,y=self._key_cursor; nx,ny=x+dx,y+dy
            if self._in_bounds(nx,ny): self._set_key_cursor((nx,ny))
            return
        if key in (Qt.Key_Return,Qt.Key_Enter) and self._key_cursor is not None:
            event.accept(); self._apply_key_brush(1); return
        if key in (Qt.Key_Delete,Qt.Key_Backspace) and self._key_cursor is not None:
            event.accept(); self._apply_key_brush(0); return
        if key==Qt.Key_Escape and self._key_cursor is not None:
            event.accept(); self._set_key_cursor(None); return
        super().keyPressEvent(event)

    def _set_key_cursor(self,pos):
        pos=None if pos is None else (int(pos[0]),int(pos[1]))
        previous=self._key_cursor
        if pos==previous:return
        self._key_cursor=pos
        z=self.zoom
        for pixel in (previous,pos):
            if pixel is not None:
                x,y=pixel; self.update(QRect(x*z-1,y*z-1,z+2,z+2))
        if pos is not None:self._scroll_key_cursor_into_view(pos)

    def _scroll_key_cursor_into_view(self,pos):
        area=self._scroll_area()
        if area is None:return
        x,y=pos; z=self.zoom; margin=z*3
        h=area.horizontalScrollBar(); v=area.verticalScrollBar()
        view=area.viewport()
        if x*z < h.value()+margin: h.setValue(max(0,x*z-margin))
        elif (x+1)*z > h.value()+view.width()-margin: h.setValue((x+1)*z-view.width()+margin)
        if y*z < v.value()+margin: v.setValue(max(0,y*z-margin))
        elif (y+1)*z > v.value()+view.height()-margin: v.setValue((y+1)*z-view.height()+margin)

    def _apply_key_brush(self,value):
        x,y=self._key_cursor
        self.document.brush(x,y,value,size=self.brush_size)
        self._patch_base_cache(self._stroke_bounds(x,y,x,y))
...
class PixelStudioWindow(QMainWindow):
    ...
    def __init__(self,path: str|Path|None=None,language: str=DEFAULT_LANGUAGE,parent=None,preferences: PreferencesStore|None=None,project_root: str|Path|None=None,project_workspace=None):
        ...
        self._preview_cache=None; self._preview_rebuilds=0; self._pending_preview_bounds=None; self._icon_buttons=[]
    ...
    def _build_ui(self):
        ...
        self.undo_btn=QPushButton(tr('action.undo')); self.undo_btn.setProperty('trKey','action.undo'); self.undo_btn.setObjectName('GhostButton'); self.undo_btn.clicked.connect(self.undo); self.redo_btn=QPushButton(tr('action.redo')); self.redo_btn.setProperty('trKey','action.redo'); self.redo_btn.setObjectName('GhostButton'); self.redo_btn.clicked.connect(self.redo); header.addWidget(self.undo_btn); header.addWidget(self.redo_btn); self._bind_button_icon(self.undo_btn,'undo','text.secondary'); self._bind_button_icon(self.redo_btn,'redo','text.secondary')
        ...
        self.save_btn=QPushButton(tr('pixel.action.save')); self.save_btn.setProperty('trKey','pixel.action.save'); self.save_btn.setObjectName('PrimaryButton'); self.save_btn.clicked.connect(self.save_png); header.addWidget(self.save_btn); layout.addWidget(header_host); self._bind_button_icon(self.save_btn,'save','accent.on_primary')
        ...
        self.canvas=PixelCanvas(self.document); self.canvas_scroll=QScrollArea(); self.canvas_scroll.setObjectName('PixelCanvasScroll'); self.canvas_scroll.viewport().setObjectName('CanvasViewport'); self.canvas_scroll.setWidget(self.canvas); self.canvas_scroll.setWidgetResizable(False); self.canvas_scroll.setFrameShape(QFrame.NoFrame)
        self.ruler_hr=PixelRulerStrip('horizontal'); self.ruler_vr=PixelRulerStrip('vertical'); self.ruler_corner=PixelRulerStrip('corner')
        ruler_grid=QGridLayout(); ruler_grid.setContentsMargins(0,0,0,0); ruler_grid.setSpacing(0)
        ruler_grid.addWidget(self.ruler_hr,0,1); ruler_grid.addWidget(self.ruler_vr,1,0); ruler_grid.addWidget(self.ruler_corner,0,0); ruler_grid.addWidget(self.canvas_scroll,1,1)
        ruler_grid.setColumnStretch(1,1); ruler_grid.setRowStretch(1,1)
        center.addLayout(ruler_grid,1); self.workspace_splitter.addWidget(canvas_frame)
        ...
        for key,fn,icon_name in [('pixel.action.copy',self.copy_selection,'copy'),('pixel.action.cut',self.cut_selection,'cut'),('pixel.action.paste',self.paste_selection,'paste')]:
            b=QPushButton(tr(key) if '.' in key else key); b.setProperty('trKey',key if '.' in key else ''); b.clicked.connect(fn); self._bind_button_icon(b,icon_name); il.addWidget(b)
        section('pixel.section.text_font'); self.insert_text_btn=QPushButton(tr('pixel.action.insert_bitmap_text')); self.insert_text_btn.setProperty('trKey','pixel.action.insert_bitmap_text'); self.insert_text_btn.clicked.connect(self.insert_bitmap_text); il.addWidget(self.insert_text_btn); self._bind_button_icon(self.insert_text_btn,'text')
        section('pixel.section.transform')
        for key,fn,icon_name in [('pixel.action.invert',self.invert,'invert'),('pixel.action.flip_h',self.flip_h,'flip-h'),('pixel.action.flip_v',self.flip_v,'flip-v'),('pixel.action.canvas_size',self.resize_canvas,'zoom-fit'),('pixel.action.rotate90',self.rotate90,'rotate'),('pixel.action.rotate180',self.rotate180,'rotate'),('pixel.action.rotate270',self.rotate270,'rotate'),('pixel.action.crop_selection',self.crop_selection,'crop'),('pixel.action.clear',self.clear,'clear')]:
            b=QPushButton(tr(key) if '.' in key else key); b.setProperty('trKey',key if '.' in key else ''); b.clicked.connect(fn); self._bind_button_icon(b,icon_name); il.addWidget(b)
        section('pixel.tab.export')
        for key,fn,icon_name in [('pixel.action.open',self.open_image,'open'),('pixel.action.save',self.save_png,'save'),('pixel.action.export_bin',self.save_bin,'export-bin'),('pixel.action.export_c',self.save_c_header,'export-h'),('pixel.action.font',self.font_generator,'font')]:
            b=QPushButton(tr(key) if '.' in key else key); b.setProperty('trKey',key if '.' in key else ''); b.clicked.connect(fn); self._bind_button_icon(b,icon_name); il.addWidget(b)
        ...
        self.canvas.documentChanged.connect(self._document_changed); self.canvas.pixelsChanged.connect(self._queue_preview_patch); self.canvas.pixelsChanged.connect(lambda _bounds:self.output_workbench.document_changed()); self.canvas.cursorChanged.connect(self._cursor_changed); self.canvas.selectionChanged.connect(lambda _v:self.refresh_preview()); self.canvas.selectionChanged.connect(lambda _v:self.output_workbench.selection_changed()); self.canvas.zoomChanged.connect(self._canvas_zoom_changed)
        self.canvas_scroll.viewport().installEventFilter(self); self.canvas.installEventFilter(self)
        self.canvas_scroll.horizontalScrollBar().valueChanged.connect(lambda _v: self._sync_rulers())
        self.canvas_scroll.verticalScrollBar().valueChanged.connect(lambda _v: self._sync_rulers())
        self.canvas.zoomChanged.connect(lambda _v: self._sync_rulers())

    def _sync_rulers(self):
        if getattr(self, 'ruler_hr', None) is None: return
        view = self.canvas_scroll.viewport()
        theme = self._resolved_theme or 'monooled-light'
        self.ruler_hr.sync(self.document.width, self.canvas.zoom,
                           self.canvas_scroll.horizontalScrollBar().value(), view.width(), theme)
        self.ruler_vr.sync(self.document.height, self.canvas.zoom,
                           self.canvas_scroll.verticalScrollBar().value(), view.height(), theme)
        self.ruler_corner.sync(0, self.canvas.zoom, 0, 0, theme)

    def _sync_ruler_visibility(self):
        visible = bool(self.preferences.get('pixel_studio.rulers', True))
        for strip in (self.ruler_hr, self.ruler_vr, self.ruler_corner):
            strip.setVisible(visible)
        self._sync_rulers()

    def _bind_button_icon(self,button,icon_name: str,color_token: str = 'text.primary'):
        """Register a button for theme-aware icons; colors refresh with the theme."""
        self._icon_buttons.append((button,icon_name,color_token))
        theme=get_theme(self._resolved_theme or 'monooled-light')
        size=self._ui_metrics.get('icon',16)
        button.setIcon(studio_icon(icon_name,theme[color_token],size)); button.setIconSize(QSize(size,size))
    ...
    def _refresh_tool_icons(self,selected: str | None = None):
        ...
        for button,icon_name,color_token in getattr(self,'_icon_buttons',[]):
            try:
                button.setIcon(studio_icon(icon_name,theme[color_token],size)); button.setIconSize(QSize(size,size))
            except RuntimeError:
                continue
    ...
    def apply_runtime_delta(self,delta):
        ...
        if delta.appearance_changed or theme_changed:
            ...
            for strip in (self.ruler_hr, self.ruler_vr, self.ruler_corner): strip.theme_name=theme; strip.update()
        if delta.pixel_changed or delta.performance_changed:
            ...
            self.document.set_max_undo(runtime.undo_history); self.preview.setVisible(runtime.actual_preview); self._sync_ruler_visibility()
    ...
    def cut_selection(self):
        ...
        self.copy_selection(); x,y,w,h=self.canvas.selection; self.document.clear_region(x,y,w,h)
        self._invalidate_preview(); self.canvas.update(); self.refresh_preview()
    def paste_selection(self):
        if not self.clipboard:return
        x,y=self.canvas.selection[:2] if self.canvas.selection else (0,0); self.document.paste_region(x,y,self.clipboard); self._invalidate_preview(); self.canvas.update(); self.refresh_preview()
    ...
    def _apply_fit_zoom(self):
        if getattr(self,'_zoom_mode','manual')=='fit': self._set_zoom_internal(self._fit_zoom())
    def _set_zoom_internal(self,value):
        """Recalculate zoom on behalf of Fit or the zoom control: the emitted
        zoomChanged must not flip the mode back to manual."""
        self._fit_recalculating=True
        try:self.set_zoom(value,from_control=False)
        finally:self._fit_recalculating=False
    def _zoom_control_changed(self,_index):
        data=self.zoom_combo.currentData(); self._zoom_mode='fit' if data=='fit' else 'manual'; self._set_zoom_internal(self._fit_zoom() if data=='fit' else int(data))
    def set_zoom(self,value,from_control=True):
        clamped=max(1,min(40,int(value)))
        if self.canvas.zoom==clamped:
            # Fit-mode refits call this with unchanged values constantly; a
            # no-op must not invalidate the pixel cache or re-emit the signal.
            return
        self.canvas.zoom=clamped; self.canvas._sync_size(); self.canvas.update()
        if from_control:self._zoom_mode='manual'
        self.canvas.zoomChanged.emit(self.canvas.zoom)
    def _canvas_zoom_changed(self,value):
        if getattr(self,'_fit_recalculating',False): self._sync_pixel_size_control(value); return
        self._zoom_mode='manual'; text=f'{value}×'; idx=self.zoom_combo.findText(text)
        if idx>=0:self.zoom_combo.setCurrentIndex(idx)
        self._sync_pixel_size_control(value)
    def _sync_pixel_size_control(self,value):
        workbench=getattr(self,'output_workbench',None)
        if workbench is None:return
        spin=workbench.pixel_size
        if spin.value()!=int(value):
            spin.blockSignals(True);spin.setValue(int(value));spin.blockSignals(False)
    def eventFilter(self,obj,event):
        if obj is self.canvas_scroll.viewport() and event.type()==QEvent.Resize:self._apply_fit_zoom(); self._sync_rulers()
    ...
    def _document_changed(self):
        self._preview_timer.stop(); self._invalidate_preview(); self._sync_rulers(); self.refresh_preview(); self.output_workbench.document_changed(); self.title=(self.path.name if self.path else self.tr('pixel.title'))+(' ●' if self.document.dirty else '')
        parent=self.parentWidget()
        while parent is not None:
    ...
    def save_c_header(self):
        ...
        return True
    def undo(self):
        if self.document.undo():self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def redo(self):
        if self.document.redo():self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def invert(self):self.document.invert(); self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def flip_h(self):self.document.flip_horizontal(); self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def flip_v(self):self.document.flip_vertical(); self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def clear(self):self.document.clear(); self._invalidate_preview(); self.canvas.invalidate_base_cache(); self.canvas.update(); self.refresh_preview()
    def resize_canvas(self):
        dialog=CanvasResizeDialog(self.document,self.tr,self)
        if dialog.exec()!=QDialog.Accepted:return
        w,h,anchor=dialog.values();self.document.resize_canvas(w,h,anchor=anchor);self._invalidate_preview();self.canvas.set_document(self.document);self._sync_rulers();self._apply_fit_zoom();self.refresh_preview()
    def rotate90(self):self.document.rotate90();self._invalidate_preview();self.canvas.set_document(self.document);self._sync_rulers();self.refresh_preview()
    def rotate180(self):self.document.rotate180();self._invalidate_preview();self.canvas.set_document(self.document);self._sync_rulers();self.refresh_preview()
    def rotate270(self):self.document.rotate270();self._invalidate_preview();self.canvas.set_document(self.document);self._sync_rulers();self.refresh_preview()
    def crop_selection(self):
        if not self.canvas.selection:return
        self.document.crop(*self.canvas.selection);self._invalidate_preview();self.canvas.selection=None;self.canvas.set_document(self.document);self.refresh_preview()
    ...
    def layout_violations(self):
        ...
        return issues
    def _queue_preview_patch(self, bounds):
        x0,y0,x1,y1=bounds
        pending=self._pending_preview_bounds
        if pending is None:
            self._pending_preview_bounds=(x0,y0,x1,y1)
        else:
            self._pending_preview_bounds=(min(pending[0],x0),min(pending[1],y0),max(pending[2],x1),max(pending[3],y1))
        self._preview_timer.start()

    def _invalidate_preview(self):
        self._preview_cache=None; self._pending_preview_bounds=None

    def _patch_preview_cache(self,pixmap,bounds):
        """Repaint the damage rect of the 2x nearest-neighbour preview."""
        x0,y0,x1,y1=bounds
        if x0<0 or y0<0 or x1>=self.document.width or y1>=self.document.height: return False
        z=2; painter=QPainter(pixmap)
        damage=QRect(x0*z,y0*z,(x1-x0+1)*z,(y1-y0+1)*z)
        painter.fillRect(damage,QColor(PIXEL_OFF_COLOR))
        painter.setPen(Qt.NoPen); painter.setBrush(QColor(PIXEL_ON_COLOR))
        for y in range(y0,y1+1):
            row=self.document.pixels[y]
            for x in range(x0,x1+1):
                if row[x]: painter.drawRect(x*z,y*z,z,z)
        painter.end(); return True

    def _update_preview_pixmap(self):
        document=self.document; scale=2
        cache=self._preview_cache
        if cache is not None and cache[1]==(document.width,document.height):
            pixmap=cache[0]
            bounds=self._pending_preview_bounds; self._pending_preview_bounds=None
            if bounds is not None: self._patch_preview_cache(pixmap,bounds)
            self.preview.setPixmap(pixmap); return
        self._pending_preview_bounds=None
        pixmap=_document_pixmap(document,scale)
        self._preview_cache=(pixmap,(document.width,document.height),scale); self._preview_rebuilds+=1
        self.preview.setPixmap(pixmap)

    def refresh_preview(self):
        if getattr(self,'_zoom_mode','manual')=='fit':self._apply_fit_zoom()
        if bool(self.preferences.get('pixel_studio.actual_preview',True)):self._update_preview_pixmap()
        else:self._invalidate_preview();self.preview.clear()
        vlsb_bytes=self.document.width*((self.document.height+7)//8)
        self.info.setText(f'{self.document.width} × {self.document.height} · {vlsb_bytes} B VLSB')
        if self.canvas.selection:
            x,y,w,h=self.canvas.selection
            lit=sum(sum(row[x:x+w]) for row in self.document.pixels[y:y+h])
            self.selection_info.setText(self.tr('pixel.selection',x=x,y=y,w=w,h=h)+' · '+self.tr('pixel.selection.lit',lit=lit))
